...
public class Book {
    ...
    @Builder
    public Book(String title, String author, String publisher, String isbn, BookStatus status, Boolean isAvailable) {
        this.title = title;
        this.author = author;
    ...
    /**
     * [워크플로우 전환 1] 일반 도서를 유휴화 분류하여 '마모 점검 대상' 목록으로 보냄
     */
    public void markAsIdle() {
        this.status = BookStatus.IDLE;
        this.isAvailable = false; // 점검 대상이 되면서 대출 불가 처리
    }

    /**
     * [워크플로우 전환 2] 마모 점검을 최초 등록(POST) 완료했을 때 '마모 처리 현황' 상태로 변경
     */
    public void completeCheckAndMoveToInProgress(String wearLevel, String soilLevel, Boolean isAvailable) {
        this.status = BookStatus.IN_PROGRESS; // '마모 처리 현황' 목록으로 이동
