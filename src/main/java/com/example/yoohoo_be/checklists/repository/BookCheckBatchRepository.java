...
public interface BookCheckBatchRepository extends JpaRepository<BookCheckBatch, Long> {
    ...
    @Query("SELECT b FROM BookCheckBatch b JOIN FETCH b.book WHERE b.book.id = :bookId ORDER BY b.checkedDate DESC, b.id DESC LIMIT 1")
    Optional<BookCheckBatch> findLatestByBookId(@Param("bookId") Long bookId);
}